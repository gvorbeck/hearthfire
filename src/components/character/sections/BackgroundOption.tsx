...
import { Radio, CheckboxGroup, Input, UseDots } from "@/components/ui";
import type { CheckboxGroupItem } from "@/components/ui";
...
const BoundedCheckboxList = ({
  choices,
  level,
  disabled,
  selected,
  onSave,
}: BoundedCheckboxListProps) => {
  const lockedValues = choices.items
    .filter((c) => c.locked)
    .map((c) => c.value);
  ...
  const checked = useMemo<Record<string, boolean>>(
    () =>
      Object.fromEntries(
        choices.items.map((c) => [
          c.value,
          c.locked || selected.includes(c.value),
        ]),
      ),
      ...
      <div className={styles.optionBody}>
        ...
        {option.freeText && (
          <Input
            ...
            disabled={!selected}
            onChange={(e) =>
              onFreeTextChange(option.freeText!.key, e.target.value)
            }
          />
        )}
      </div>
