...
describe('UseDots', () => {
  ...
  it('labels filled dots as clearable and empty dots as markable', () => {
    render(<UseDots total={3} checked={1} onChange={vi.fn()} />);
    // First dot is filled (checked = 1), the rest are empty.
    expect(screen.getByRole('button', { name: 'Clear use 1' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Mark use 2' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Mark use 3' })).toBeInTheDocument();
  });

  it('clicking an empty dot fills up to and including that dot', () => {
    const onChange = vi.fn();
    render(<UseDots total={4} checked={1} onChange={onChange} />);
    fireEvent.click(screen.getByRole('button', { name: 'Mark use 3' }));
  ...
  it('clicking a filled dot clears down to just below it', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Clear use 2' }));
  ...
  it('clicking the only filled dot clears all uses', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Clear use 1' }));
  ...
  it('does not fire onChange while disabled', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Mark use 2' }));
